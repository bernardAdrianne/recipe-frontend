<script lang="ts">
  ...
  let hovered: 'signin' | 'signup' = 'signin';
  ...
  async function handleLogin(event: Event) {
    ...
    try {
      ...
      const res = await fetch('https://airecipe-backend-2.onrender.com/auth/signin', {
        method: 'POST',
...
<div 
  ...
  class="relative  border border-gray-300 bg-white rounded-3xl shadow-2xl overflow-hidden w-full max-w-5xl flex flex-col md:flex-row">
  <!-- LEFT SIDE: FORM -->
  ...
  <div class="flex justify-center mb-6">
    <div
      class="relative flex bg-gray-100 rounded-md p-2 shadow-inner w-full max-w-xl overflow-hidden"
    >
  ...
  <div
    class="
      ...
      pointer-events-none
    "
    class:translate-x-full={hovered === 'signup'}
  ></div>

  <!-- SIGN IN -->
  <button
    type="button"
    on:mouseenter={() => hovered = 'signin'}
    on:mouseleave={() => hovered = activeTab}
    ...
    class:text-black={activeTab === 'signin' || hovered === 'signin'}
    class:text-gray-700={activeTab !== 'signin' && hovered !== 'signin'}
      >
  ...
  <button
    ...
    class="
      ...
      text-sm font-semibold
      transition-colors duration-300"
    class:text-black={activeTab === 'signup' || hovered === 'signup'}
    class:text-gray-700={activeTab !== 'signup' && hovered !== 'signup'}
    >
    <UserPlus size="16" />
    Sign Up
  </button>
  ...
  <div class="space-y-2">
    ...
    <label 
      ...
      Email Address
      {#if !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)}
        <span class="text-red-500 mr-5">*</span>
      {/if}
    </label>
  ...
  <p class="text-center text-sm text-gray-500 mt-6">
    Don’t have an account? 
      <span class="text-[#0f1b2d] font-semibold cursor-pointer hover:underline" on:click={navigateToSignup}>
        Sign Up
      </span>
  </p>
