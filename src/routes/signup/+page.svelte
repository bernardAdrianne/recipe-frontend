<script lang="ts">
    ...
    async function handleRegister(event: Event) {
        ...
        try {
            const res = await fetch('https://recipe-backend-4-csnw.onrender.com/api/auth/signup', {
                method: 'POST',
                headers: {
...
<div class="flex flex-col items-center justify-center h-screen">
    <!-- recipinoy logo -->
    <h1 class="w-28 h-24 mb-4 font-bold text-2xl text-[#115D33]">ReciPinoy</h1>
    ...
    <form action="" class="w-90 mb-10 mt-8" on:submit|preventDefault={handleRegister}>
        ...
        <!-- submit button -->
        <button class="bg-[#228B22] text-[#FFFFFF] font-medium text-center text-sm rounded-4xl p-4 w-full cursor-pointer hover:bg-[#00AB41] transition duration-200 ease-in-out"
            type="submit">
            Create Account
        </button>
    </form>
    ...
    <p class="text-sm text-[#A5A4A1] mt-24">Don't have an account? 
        <span class="text-[#050301] cursor-pointer font-semibold" on:click={navigateToSignin}>Signin</span>
    </p>
</div>